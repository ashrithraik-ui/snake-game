...
#screen setup
wn = turtle.Screen()
wn.title("snake game by ashrith Rai k")
wn.bgcolor("green")
...
head.shape("square")
head.color("purple")
head.penup()#not to draw a line when it moves
head.goto(0,0)
head.direction = "stop"
...
pen.hideturtle()
pen.goto(0,260)
pen.write("Score:0 Highscore:0",align="center",font=("courrier",24,"normal"))
...
def move():
    if head.direction == "up":
        y = head.ycor()
        head.sety(y + 20)#move snakes head 20 pixels upward
    if head.direction == "down":
        y = head.ycor()
        head.sety(y - 20)
    if head.direction == "left":
...
while True:
    ...
    #check for a collision with the bordee
    if head.xcor()>390 or head.xcor()<-390 or head.ycor()>290 or head.ycor()<-290:
        time.sleep(1)#1second time pause
        head.goto(0,0)
        head.direction = "stop"
    ...
    time.sleep(delay)#delays the execution
